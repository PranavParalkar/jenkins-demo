...
type LaserFlowProps = {
  className?: string;
  style?: CSSProperties;
  transparent?: boolean; // make canvas background transparent
  clearColor?: string; // hex color for clear when not transparent
  clearAlpha?: number; // alpha for clear color
  wispDensity?: number;
  dpr?: number;
...
const LaserFlow = ({
  ...
  const cc = new THREE.Color(clearColor || "#000000");
  renderer.setClearColor(cc, transparent ? (clearAlpha ?? 0) : (clearAlpha ?? 1));
  ...
  }, [dpr, flowSpeed, fogIntensity, fogScale, mouseSmoothTime, mouseTiltStrength, wispDensity, wispIntensity, wispSpeed, flowStrength, decay, falloffStart, fogFallSpeed, horizontalBeamOffset, verticalBeamOffset, verticalSizing, horizontalSizing, transparent, clearColor, clearAlpha]);
  ...
  return <div ref={mountRef} className={`relative w-full h-full ${className || ""}`} style={style} />;
};
